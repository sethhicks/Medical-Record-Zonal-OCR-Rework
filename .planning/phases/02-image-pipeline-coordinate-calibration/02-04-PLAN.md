...
must_haves:
  truths:
    - "UB04_FIELDS contains exactly 24 FieldDef entries"
    - "UB04_TABLE_FIELDS contains exactly 7 TableFieldDef entries each with exactly 22 row_boxes"
    - "Every FieldDef has a non-empty name, valid 4-tuple box, integer psm, and string label"
  artifacts:
    - path: "config/ub04.py"
      provides: "UB04_FIELDS (24 FieldDef) and UB04_TABLE_FIELDS (7 TableFieldDef)"
      contains: "UB04_FIELDS"
  key_links:
    - from: "config/ub04.py"
      to: "pipeline/calibrate.py"
      via: "import UB04_FIELDS, UB04_TABLE_FIELDS"
      pattern: "from config.ub04 import"
---

<objective>
Populate config/ub04.py with all 24 FieldDef entries and 7 TableFieldDef entries (22 revenue line rows each) using the estimated pixel coordinates from RESEARCH.md Section 5.

Purpose: UB-04 coordinates are the sole source of truth (D-06) for UB-04 field regions. These drive both the calibration overlay and the Phase 4 extractor. Per D-05, all coordinates are starting estimates requiring visual verification via calibrate.py against test.pdf.

Output: config/ub04.py fully populated; test_ub04_fields_populated and test_ub04_table_fields_row_count pass green.
</objective>
...
<task type="auto">
  <name>Task 1: Populate UB04_FIELDS and UB04_TABLE_FIELDS</name>
  <files>config/ub04.py</files>

  <read_first>
    - config/ub04.py (current state — empty lists, import already present)
    - config/base.py (FieldDef and TableFieldDef dataclass schemas — field order)
    - .planning/phases/02-image-pipeline-coordinate-calibration/02-RESEARCH.md §5.1 (UB04_FIELDS table — 24 entries) and §5.2 (UB04_TABLE_FIELDS — 7 sub-fields, 22 row y-ranges table)
  </read_first>

  <action>
Replace config/ub04.py entirely with the populated version. Keep the existing module docstring and import; replace the two empty lists with fully populated versions.
...
Write the complete file above verbatim. There are exactly 24 FieldDef entries in UB04_FIELDS and exactly 7 TableFieldDef entries in UB04_TABLE_FIELDS, each with exactly 22 row_boxes tuples.
  </action>

  <verify>
    <automated>cd /c/Users/shset/Documents/GitHub/OCR-Rework && python -m pytest tests/test_phase2.py::test_ub04_fields_populated tests/test_phase2.py::test_ub04_table_fields_row_count -v</automated>
  </verify>

  <acceptance_criteria>
    - `python -c "from config.ub04 import UB04_FIELDS, UB04_TABLE_FIELDS; print(len(UB04_FIELDS), len(UB04_TABLE_FIELDS))"` prints `24 7`
    - `python -m pytest tests/test_phase2.py::test_ub04_fields_populated -v` exits 0 (PASSED)
    - `python -m pytest tests/test_phase2.py::test_ub04_table_fields_row_count -v` exits 0 (PASSED)
    - `python -c "from config.ub04 import UB04_TABLE_FIELDS; print(all(len(f.row_boxes)==22 for f in UB04_TABLE_FIELDS))"` prints `True`
    - `grep -v "^#" config/ub04.py | grep -c "FieldDef("` returns at least 24
  </acceptance_criteria>

  <done>UB04_FIELDS has 24 FieldDef entries, UB04_TABLE_FIELDS has 7 TableFieldDef entries each with 22 row_boxes. Both tests pass green.</done>
</task>
...
|----------|-------------|
| config/ub04.py → pipeline/calibrate.py | Coordinate tuples are static Python literals; no external input |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-2-04-01 | Information Disclosure | config/ub04.py — coordinate definitions | accept | Coordinates describe standard published form layout (UB-04 CMS-1450); not sensitive. Checked into version control intentionally. |
</threat_model>

<verification>
```bash
cd /c/Users/shset/Documents/GitHub/OCR-Rework
python -m pytest tests/test_phase2.py::test_ub04_fields_populated tests/test_phase2.py::test_ub04_table_fields_row_count -v
python -c "from config.ub04 import UB04_FIELDS, UB04_TABLE_FIELDS; assert len(UB04_FIELDS)==24; assert len(UB04_TABLE_FIELDS)==7; assert all(len(f.row_boxes)==22 for f in UB04_TABLE_FIELDS); print('OK')"
```
Expected: both tests PASSED, assertion script prints OK.
</verification>

<success_criteria>
- UB04_FIELDS: 24 FieldDef entries
- UB04_TABLE_FIELDS: 7 TableFieldDef entries, each with exactly 22 row_boxes
- test_ub04_fields_populated passes
- test_ub04_table_fields_row_count passes
</success_criteria>

<output>
After completion, create `.planning/phases/02-image-pipeline-coordinate-calibration/02-04-SUMMARY.md`
</output>
